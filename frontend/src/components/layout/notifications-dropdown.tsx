...
export function NotificationsDropdown({
  ...
  const [hideReadNotifications, setHideReadNotifications] =
    useState<boolean>(false); // Toggle for hiding read notifications
    ...
    if (lastMessageData) {
      try {
        ...
        if (
          ...
          console.log(
            "NotificationsDropdown: Received WebSocket notification update, refreshing..."
          );

          // FIXED: Add debouncing and prevent recursive calls
        ...
        console.debug(
          "NotificationsDropdown: WebSocket message parsing error (likely not a notification):",
          error
        );
      }
    }
  ...
  const handleItemClick = async (notification: Notification) => {
    try {
      ...
      if (
        !["follow_request", "group_invite", "group_join_request"].includes(
          notification.type
        )
      ) {
        switch (notification.type) {
          ...
          case "group_event_created":
            if (
              ...
              console.log(
                `Navigate to event: ${notification.entity_id} - Group ID needed.`
              );
  ...
  const handleAction = async (
    ...
    try {
      ...
      toast.error(
        actionError?.message || error?.message || "Could not complete action."
      );
    } finally {
  ...
  return (
    ...
    >
      <DropdownHeader
        className="flex justify-between items-center w-full p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-zinc-800 dark:to-zinc-700 border-b border-zinc-200 dark:border-zinc-600"
        style={{
          width: "384px",
          minWidth: "384px",
          maxWidth: "384px",
      ...
      <div
        ...
        style={{ width: "384px", boxSizing: "border-box" }}
      >
          ...
          return filteredNotifications.length === 0 ? (
            ...
            filteredNotifications.map((notification, index) => {
              ...
              const canPerformAction =
                (acceptUrl || declineUrl) && !notification.is_read;
                ...
                <Fragment key={notification.id}>
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={() =>
                        !isProcessing && handleItemClick(notification)
                      }
                    >
                      {/* Notification Icon */}
                      <div
                        className={`shrink-0 p-2 rounded-full transition-all duration-200 group-hover:scale-105 ${
                          !notification.is_read
